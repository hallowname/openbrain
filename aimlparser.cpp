...
bool Node::match(QStringList::const_iterator input, const QStringList &inputWords, const QString &currentThat, const QString &currentTopic, QStringList &capturedThatTexts, QStringList &capturedTopicTexts, Leaf *&leaf)
{
...
void AIMLParser::parseCategory(QDomNode* categoryNode)
{
  ...
  for ( QStringList::ConstIterator it = words.begin(); it != words.end(); ++it )
  {
	...
	else if ((*it != "_") && whereToInsert->childs.count() && (whereToInsert->childs.at(0)->word == "_"))
	  index = 1;
...
QString AIMLParser::resolveNode(QDomNode* node, const QStringList &capturedTexts, const QStringList &capturedThatTexts, const QStringList &capturedTopicTexts)
{
  QString result("");
...
QString AIMLParser::getResponse(QString input, const bool &srai)
{
  ...
  while (true)
  {
    *sentence = (*sentence).toLower();
    ...
    if (!root.match(it, inputWords, thatList.count() && thatList[0].count() ? thatList[0][0] : QString(""), curTopic, capturedThatTexts, capturedTopicTexts, leaf))
...
void AIMLParser::run()
{
  loadVars(QDir::homePath() + "/.openbrain/utils/vars.xml", false);
  loadVars(QDir::homePath() + "/.openbrain/utils/bot.xml", true);
  loadSubstitutions(QDir::homePath() + "/.openbrain/utils/substitutions.xml");
  ...
  {
    emit aimlLoad_doc(file);
    emit aimlLoad_percent(i);

    loadAiml(dirname + "/" + file);
